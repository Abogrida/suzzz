...
export default function EmployeesPage() {
    ...
    const handleUpdate = async () => {
        ...
        const res = await fetch(`/api/employees/${editingEmp.id}`, {
            ...
            body: JSON.stringify({ name: editName.trim(), password: editPass.trim() })
        });
        setSaving(false);
        if (res.ok) {
            setToast({ msg: 'تم تحديث بيانات الموظف', type: 'success' });
            setEditingEmp(null);
            load();
        } else {
            const r = await res.json();
            setToast({ msg: r.error || 'فشل التحديث', type: 'error' });
        }
    };
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
                    <div style={{ background: '#fff', borderRadius: 20, padding: 30, width: '100%', maxWidth: 450, boxShadow: '0 20px 50px rgba(0,0,0,0.2)' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                            <div>
                                <label style={{ display: 'block', fontWeight: 700, marginBottom: 8, fontSize: 15, color: '#475569' }}>الاسم</label>
                                <input style={inp} value={editName} onChange={e => setEditName(e.target.value)} />
                            </div>
                            <div>
                                <label style={{ display: 'block', fontWeight: 700, marginBottom: 8, fontSize: 15, color: '#475569' }}>كلمة المرور الجديدة</label>
                                <input style={inp} value={editPass} onChange={e => setEditPass(e.target.value)} />
                            </div>
                            <div style={{ display: 'flex', gap: 12, marginTop: 10 }}>
                                <button onClick={handleUpdate} disabled={saving} style={{ flex: 1, background: '#6366f1', color: '#fff', border: 'none', borderRadius: 14, padding: '14px', fontWeight: 800, fontSize: 16, cursor: 'pointer', fontFamily: 'Cairo' }}>
                        ...
                        <div key={emp.id} style={{ background: '#fff', borderRadius: 18, boxShadow: '0 4px 15px rgba(0,0,0,0.05)', overflow: 'hidden', border: '1.5px solid #e2e8f0', display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div style={{ padding: '18px 22px', flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                <div>
                                    ...
                                    <div style={{ fontSize: 18, fontWeight: 900, color: '#1e293b', fontFamily: 'Cairo', letterSpacing: showPass === emp.id ? 1 : 4 }}>
                                        {showPass === emp.id ? emp.password : '••••••'}
                                    </div>
                                </div>
                                <button onClick={() => setShowPass(showPass === emp.id ? null : emp.id)} style={{ background: '#f1f5f9', border: 'none', borderRadius: 10, padding: '8px 14px', cursor: 'pointer', fontSize: 14, fontFamily: 'Cairo', color: '#6366f1', fontWeight: 800 }}>
