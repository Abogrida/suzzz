...
export default function MainWarehousePage() {
    ...
    const load = async () => {
        setLoading(true);
        const [p, c] = await Promise.all([fetch('/api/products?warehouse=main').then(r => r.json()), fetch('/api/categories').then(r => r.json())]);
    ...
    const handleSave = async () => {
        setSaving(true);
        const body = { ...form, warehouse: 'main' };
    ...
    return (
        <div className="page-content" style={{ direction: 'rtl', minHeight: '100vh' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, fontSize: 14, color: '#64748b', fontWeight: 600 }}>
                <span onClick={() => router.push('/products')} style={{ cursor: 'pointer', color: '#6366f1', fontWeight: 700 }}>🏭 المخزن</span>
                <span>{'›'}</span>
                <span style={{ color: '#1e293b' }}>🏛️ المخزن الرئيسي</span>
            </div>

            <h1 className="page-title" style={{ marginBottom: 20 }}>🏛️ المخزن الرئيسي</h1>
            ...
            <div className="card" style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
                ...
                <a href="/api/export/products/excel?warehouse=main" className="btn btn-success" style={{ background: '#059669', textDecoration: 'none' }}>📥 تصدير</a>
                        ...
                        <table className="data-table">
                            ...
                            <tbody>
                                ...
                                {filtered.length === 0 && <tr><td colSpan={9} style={{ textAlign: 'center', padding: '60px 0', color: '#94a3b8' }}>
                                    <div style={{ fontSize: 44, marginBottom: 12 }}>📭</div><div style={{ fontWeight: 800, fontSize: 18, color: '#374151' }}>{filterMode !== 'all' ? 'لا توجد منتجات في هذا الفلتر' : 'لا توجد منتجات'}</div>
                                </td></tr>}
                            </tbody>
                        </table>
                    </div>}
            ...
            {stockModal && <div onClick={() => setStockModal(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 12 }}>
                <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 20, width: '90%', maxWidth: 440, boxShadow: '0 30px 70px rgba(0,0,0,0.25)' }}>
                    ...
                    <div style={{ padding: 22, display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        <div><label style={{ display: 'block', fontWeight: 800, marginBottom: 8, fontSize: 15 }}>ملاحظات</label><input value={stockNotes} onChange={e => setStockNotes(e.target.value)} placeholder="سبب..." style={S} /></div>
                    </div>
                    <div style={{ padding: '0 22px 22px', display: 'flex', gap: 12 }}>
